...
CREATE OR REPLACE FUNCTION public.insert_look_and_relations(id_user integer, gender integer, url_image text, id_products integer[], id_offers integer[], id_styles integer[], id_offer integer[])
 RETURNS void
 LANGUAGE plpgsql
AS $function$DECLARE
    new_look_id INT;
...
BEGIN
    ...
    FOR i IN 1..array_length(id_products, 1) LOOP
        -- Inserir o produto e o id_offer correspondente
        FOR j IN 1..array_length(id_offers, 1) LOOP
            INSERT INTO looks_has_products (id_look, id_product, id_offer)
            VALUES (new_look_id, id_products[i], id_offers[j]);
        END LOOP;
    END LOOP;
